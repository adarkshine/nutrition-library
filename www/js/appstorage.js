...
function onBodyLoad(){
	...
	$("#remove").live("click",function(e){
		...
		$(this).closest("li").slideUp(function(){
			...
			window.localStorage.removeItem('names_'+index);
			
			// rearrange localstorage array 
			for(i=0; i<window.localStorage.length; i++) {
			  if( !window.localStorage.getItem("names_"+i)) {
				window.localStorage.setItem("names_"+i, window.localStorage.getItem('names_' + (i+1) ) );
				window.localStorage.removeItem('names_'+ (i+1) );
			  }
			}
...
function create_new_list(){
	for (var i = 0; i < window.localStorage.length; i++){
		todo = window.localStorage.getItem('names_'+i);
...
function save_todo(){
	...
	if(todo.length){
		...
		window.localStorage['names_'+i] = todo;
